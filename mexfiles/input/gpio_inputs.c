...
#define S_FUNCTION_NAME                gpio_inputs
...
/* Output Port  0 */
#define OUT_PORT_0_NAME                readVal
...
extern void gpio_inputs_Outputs_wrapper(const uint8_T *port_ascii,
  const uint8_T *pin,
  const uint8_T *pull,
  boolean_T *readVal);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  const uint8_T *pull = (uint8_T *) ssGetInputPortRealSignal(S, 2);
  boolean_T *readVal = (boolean_T *) ssGetOutputPortRealSignal(S, 0);
  gpio_inputs_Outputs_wrapper(port_ascii, pin, pull, readVal);
}
